...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Generate masks for set of images to exclude objects like cars, persons, animals."
    )
...
if __name__ == "__main__":
    ...
    if len(args.mask_categories) > 0:
        ...
        for filename in tqdm(
            os.listdir(IMAGE_FOLDER), desc="Masking images", unit="images"
        ):
            ...
            cv2.imwrite(
                os.path.join(IMAGE_FOLDER, f"dynamic_mask_{basename}.png"),
                (output_mask * 255).astype(np.uint8),
            )
