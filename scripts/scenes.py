...
def ours_mesh(name, up=[0, 1, 0], infolder=True):
    return {
        "data_dir": os.path.join(SDF_DATA_FOLDER, f"{name}")
        if infolder
        else SDF_DATA_FOLDER,
        "dataset": f"{name}.obj",
        "up": up,
    }
...
def ours_image(name, infolder=True):
    data_dir = (
        os.path.join(IMAGE_DATA_FOLDER, f"{name}") if infolder else IMAGE_DATA_FOLDER
    )
    dataset = f"{name}.bin"
    ...
    return {"data_dir": data_dir, "dataset": dataset}
...
def ours_volume(name, ds):
    return {"data_dir": os.path.join(VOLUME_DATA_FOLDER, f"{name}"), "dataset": ds}
...
def setup_colored_sdf(testbed, scene, softshadow=True):
    if scene == "lizard":
        ...
        if scene == "bearded_man":
            ...
            testbed.look_at = [0.500, 0.500, 0.500]
            testbed.fov, testbed.aperture_size, testbed.slice_plane_z = (
                39.600,
                0.000,
                0.377,
            )
