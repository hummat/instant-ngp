...
def parse_args():
    ...
    parser.add_argument(
        "--white_transparent", action="store_true", help="White is transparent"
    )
    parser.add_argument(
        "--black_transparent", action="store_true", help="White is transparent"
    )
...
if __name__ == "__main__":
    ...
    IMAGE_FOLDER = "."
    img_files = [[], [], []]
    img_files[0] = sorted(glob.glob(os.path.join(IMAGE_FOLDER, "rgb", "0_*.png")))
    img_files[1] = sorted(glob.glob(os.path.join(IMAGE_FOLDER, "rgb", "1_*.png")))
    img_files[2] = sorted(glob.glob(os.path.join(IMAGE_FOLDER, "rgb", "2_*.png")))
    ...
    if image.shape[2] == 3 or (image.shape[2] == 4 and image[0][0][3] != 0):
        ...
        print(
            "transparent alpha channel detected, first pixel alpha = ", image[0][0][3]
        )
    ...
    for itype in [0, 1, 2]:
        ...
        for img_f in img_files[itype]:
            pose_f = os.path.join(
                IMAGE_FOLDER,
                "pose",
                os.path.splitext(os.path.basename(img_f))[0] + ".txt",
            )
