...
sys.path += [
    os.path.dirname(pyd)
    for pyd in glob.iglob(os.path.join(ROOT_DIR, "build*", "**/*.pyd"), recursive=True)
]
sys.path += [
    os.path.dirname(pyd)
    for pyd in glob.iglob(os.path.join(ROOT_DIR, "build*", "**/*.so"), recursive=True)
]
...
def diagonally_combine_two_images(img1, img2, x_threshold, angle, gap=0, color=1):
    ...
    if gap > 0:
        rr, cc, val = weighted_line(
            0,
            int(x_threshold * img1.shape[1] - (angle * img1.shape[0] / 2)),
            img1.shape[0] - 1,
            int(x_threshold * img1.shape[1] + (angle * img1.shape[0] / 2)),
            gap,
        )
        result[rr, cc, :] = (
            result[rr, cc, :] * (1 - val[..., np.newaxis])
            + val[..., np.newaxis] * color
        )
    return result
...
def compute_error_img(metric, img, ref):
    ...
    if metric == "MAE":
        ...
        pixels_per_degree = (
            monitor_distance * (monitor_resolution_x / monitor_width) * (np.pi / 180)
        )
        ...
        result = flip.compute_flip(
            flip.utils.HWCtoCHW(ref_srgb),
            flip.utils.HWCtoCHW(img_srgb),
            pixels_per_degree,
        )
