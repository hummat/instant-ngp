...
def parse_args():
    ...
    parser.add_argument(
        "--mode", default="", type=str, help=argparse.SUPPRESS
    )  # deprecated
    parser.add_argument(
        "--network",
        default="",
    ...
    parser.add_argument(
        ...
        type=int,
        default=1000,
        help="Number of steps to train in NeRF mode before switching to RFL mode. Default is 1000. Only used if --train_mode is set to 'rfl'.",
    )
    parser.add_argument(
        "--rflrelax_begin_step",
        type=int,
        default=15000,
        help="First training step in which RflRelax mode is used. Default is 15000. Only used if --train_mode is set to 'rflrelax'.",
    )
    parser.add_argument(
        "--rflrelax_end_step",
        type=int,
        default=30000,
        help="Last training step in which RflRelax mode is used. Default is 30000. Only used if --train_mode is set to 'rflrelax'.",
    )
...
if __name__ == "__main__":
    ...
    if args.nerf_compatibility:
        ...
        if testbed.nerf.training.train_mode != ngp.TrainMode.Nerf:
            print(
                f"Warning: forcing train mode to NeRF for nerf compatibility (was {testbed.nerf.training.train_mode})"
            )
        testbed.nerf.training.train_mode = ngp.TrainMode.Nerf
    ...
    if args.test_transforms:
        ...
        with tqdm(
            range(testbed.nerf.training.dataset.n_images),
            unit="images",
            desc="Rendering test frame",
        ) as t:
            for i in t:
    ...
    if args.video_camera_path:
        ...
        for i in tqdm(
            list(range(min(n_frames, n_frames + 1))),
            unit="frames",
