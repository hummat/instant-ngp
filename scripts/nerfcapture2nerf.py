...
def parse_args():
    ...
    parser.add_argument(
        "--save_path",
        required="--stream" not in sys.argv,
        type=str,
    ...
    parser.add_argument(
        ...
        default=10.0,
        type=float,
        help="Depth scale used when saving depth. Only used when saving dataset.",
    )
    parser.add_argument(
        "--overwrite", action="store_true", help="Rewrite over dataset if it exists."
    )
...
def live_streaming_loop(reader: DataReader, max_cameras: int):
    ...
    while testbed.frame():
        ...
        if sample:
            ...
            # Transform
            X_WV = (
                np.asarray(sample.transform_matrix, dtype=np.float32)
                .reshape((4, 4))
                .T[:3, :]
                .copy()
            )
...
def dataset_capture_loop(
    reader: DataReader, save_path: Path, overwrite: bool, n_frames: int
):
    if save_path.exists():
        if overwrite:
    ...
    while True:
        ...
        if sample:
            ...
            if sample.has_depth:
                ...
                cv2.imwrite(
                    str(images_dir.joinpath(f"{total_frames}.depth.png")), depth
                )
...
if __name__ == "__main__":
    ...
    if args.stream:
        ...
        dataset_capture_loop(
            reader, Path(args.save_path), args.overwrite, args.n_frames
        )
